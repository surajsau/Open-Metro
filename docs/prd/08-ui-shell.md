...
| UI-19 | **Endless mode beat-your-best indicator:** while in Endless mode with a non-zero prior best score for the city, the HUD score display renders the personal best alongside the running score (e.g., "312 / best 280"). Once the running score exceeds the best, the display switches to a "New best!" indicator in place of the prior-best number. The best is read from localStorage at run start and held constant for the duration of the run (it is not updated live mid-run — only at run end per [UI-07](08-ui-shell.md#run-lifecycle--modes)). This surfaces tension in Endless mode without changing any simulation or scoring rules. |
...
| UI-17 | **Locked line chip countdown:** each *locked* chip displays the number of weeks remaining until that slot unlocks (derived from the current week, `lineSlots`, and the unlock schedule from [ENG-12](02-game-engine.md#weekly-reward-tick) and [GD-35](01-game-design.md#rewards--progression)). The label renders as a small integer inside the dim chip (e.g. "2"). Chips whose slot would unlock at the next weekly reward show "1". When the unlock week cannot be determined (e.g., the slot is permanently locked because the player has not yet reached the even-week unlock tier), show no countdown. This is derived data only — no new state field. |
...
| UI-18 | **Reward modal contextual hints:** each upgrade card in the reward modal appends a one-line inventory hint derived from current game state. Examples: tunnels card shows "You have N tunnel(s) free" (tunnelsFree from snapshot); carriage card shows "Your fullest train holds N/M passengers"; line card shows "You have N line(s) in use". The hint text is factual and concise (≤ 6 words). No hint is shown when the information would be trivially obvious (e.g., the player has 0 tunnels — the card already signals urgency). Hints must never reveal information the player couldn't derive themselves from the HUD in two glances. |
...
| UI-20 | **Fast-forward reward bypass:** when `?ff=S` is active, weekly reward modals that would freeze the sim ([ENG-10](02-game-engine.md#time--clock)) are auto-dismissed by selecting the first reward option. This is a dev-only affordance; it must only apply during the `?ff` pre-paint loop, not during normal gameplay. The auto-selection must consume the RNG in the same order as a player choosing option 0, so determinism ([ENG-13](02-game-engine.md#determinism--rng)) is preserved: `?seed=N&ff=S` produces the same state as manually fast-forwarding N seeds. |

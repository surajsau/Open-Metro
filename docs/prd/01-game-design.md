...
| GD-43 | **Pick up and re-deploy a deployed train.** While paused (speed = 0), clicking a deployed train sprite on the canvas picks it up — the locomotive plus its attached carriages return to inventory — and the same gesture drops it onto a **different** line, entering service there exactly as an inventory locomotive drop does ([TRN-12](05-transit-simulation.md#hardware-deployment)). Onboard passengers offload to the nearest station of the train's old line, exactly as line deletion handles them (GD-21). If the player clicks a deployed train while the game is running (speed > 0), nothing is picked up and a toast reads "Pause the game to deploy trains" — the same gate and string as GD-42. |
| GD-44 | **Re-deploy is a relocation, not a no-op.** Picking a train up and dropping it back onto its **own** source line is not a meaningful move (the train was already on that line) and is rejected — the source line is excluded as a drop target. Releasing on the source line, on empty canvas, or on any non-line target cancels harmlessly: the train stays where it was, hardware conservation (TRN-14) is preserved, and no toast error is required (the gesture simply does nothing). Only a drop onto a line *other than* the source commits a re-deploy. |
...
| ID | Rule |
|----|------|
| GD-36 | Three cities set the stage: **London** (1 river — the Thames, flowing west-to-east across the lower half of the map, gently curving; 4 starting tunnels, gentlest pacing — difficulty ●○○), **Mumbai** (western Arabian Sea coastline running roughly north-south on the left edge + a harbour inlet (Thane Creek) cutting diagonally inland from the south-east; 3 tunnels — ●●○), **Tokyo** (two rivers — the Sumida running broadly north-to-south on the right half, and the Tama/Arakawa running north-to-south on the left half — creating three city strips; 3 tunnels, fastest pacing — ●●●). Water polylines are geometry-inspired by real geography: accurate enough to be recognisable, distorted as needed to fill the 1600×1000 world canvas and create interesting topology. |
